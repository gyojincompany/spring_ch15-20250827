...
public class StudentValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if(id.strip().isEmpty() || id == null) { //두 조건 중에 한개라도 참이면 error
			...
			errors.rejectValue("id", "trouble001"); //(에러가 발생한 필드 이름, 에러코드(메시지))
		}
		
		if(pw.strip().isEmpty() || pw == null) { //두 조건 중에 한개라도 참이면 error
			System.out.println("에러가 발생된 비밀번호 : " + id);
			errors.rejectValue("pw", "trouble002"); //(에러가 발생한 필드 이름, 에러코드(메시지))
		}
		
		if(age > 19 || age <0) { //나이가 19세 초과일 경우 학생회원 자격 없음->error, 나이값이 음수일 경우 error
			errors.rejectValue("age", "trouble003"); //(에러가 발생한 필드 이름, 에러코드(메시지))
		}
	}
